...
- ✅ **원장 칼럼**: SEO 에디터, `Article`/`MedicalWebPage`/`FAQPage` 스키마, E-E-A-T 저자 박스(사진+프로필 링크), 조회수, 시드 3건
- ✅ **칼럼 퍼널 연결 (§S20)**: 홈 히어로 원장칼럼·비포애프터 바로가기 버튼(모바일 최적화), 칼럼 하단 '함께 보면 좋은 글' 3카드(관리자 수동 지정 `relatedSlugs` 우선 + 같은 카테고리 최신순 자동), 카테고리별 진료 페이지 자동 연결 CTA(진료소개/예약/전화), 진료 페이지 하단 같은 분야 최신 칼럼 3개, 칼럼 목록 `?cat=` 필터, 비포애프터↔칼럼 상호 역링크, 칼럼 대표이미지 OG 썸네일(카톡/인스타 미리보기), RSS 피드(`/rss.xml`)
