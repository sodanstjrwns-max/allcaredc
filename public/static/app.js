...
(function () {
  ...
  window.initRegionAutocomplete = function (inputId, listId) {
    ...
    var DB = [
      ...
      '서울특별시 중구 광희동', '서울특별시 중구 황학동', '서울특별시 중구 흥인동', '서울특별시 중구 다산동',
      '서울특별시 중구 동호로', '서울특별시 중구 필동', '서울특별시 중구 묵정동', '서울특별시 중구 충무로',
      // ── 성동구 (인접 생활권) ──
      '서울특별시 성동구 옥수동', '서울특별시 성동구 금호동', '서울특별시 성동구 행당동', '서울특별시 성동구 왕십리',
      '서울특별시 성동구 응봉동', '서울특별시 성동구 마장동', '서울특별시 성동구 사근동', '서울특별시 성동구 성수동',
      '서울특별시 성동구 하왕십리동', '서울특별시 성동구 도선동',
      // ── 용산구 ──
      '서울특별시 용산구 한남동', '서울특별시 용산구 이태원동', '서울특별시 용산구 보광동', '서울특별시 용산구 후암동',
      '서울특별시 용산구 청파동', '서울특별시 용산구 효창동', '서울특별시 용산구 갈월동',
      // ── 동대문구 ──
      '서울특별시 동대문구 신설동', '서울특별시 동대문구 용두동', '서울특별시 동대문구 제기동', '서울특별시 동대문구 청량리동',
      '서울특별시 동대문구 전농동', '서울특별시 동대문구 답십리동',
      // ── 광진구 ──
      '서울특별시 광진구 자양동', '서울특별시 광진구 구의동', '서울특별시 광진구 화양동', '서울특별시 광진구 군자동',
      '서울특별시 광진구 능동', '서울특별시 광진구 중곡동',
      // ── 종로/강남/송파 등 서울 주요 ──
      '서울특별시 종로구 종로', '서울특별시 종로구 명륜동', '서울특별시 종로구 혜화동',
      '서울특별시 강남구 역삼동', '서울특별시 강남구 삼성동', '서울특별시 강남구 대치동', '서울특별시 강남구 논현동',
      '서울특별시 서초구 서초동', '서울특별시 서초구 반포동',
      '서울특별시 송파구 잠실동', '서울특별시 송파구 방이동', '서울특별시 송파구 가락동',
      '서울특별시 마포구 합정동', '서울특별시 마포구 망원동', '서울특별시 마포구 상암동',
      '서울특별시 영등포구 여의도동', '서울특별시 강동구 천호동', '서울특별시 중랑구 면목동', '서울특별시 노원구 상계동',
      // ── 경기 — 안산(가이드 예시) ──
      '안산시 상록구 초지동', '안산시 상록구 사동', '안산시 상록구 본오동', '안산시 상록구 이동',
      '안산시 단원구 고잔동', '안산시 단원구 원곡동', '안산시 단원구 선부동', '안산시 단원구 와동',
      // ── 경기 주요 ──
      '성남시 분당구 정자동', '성남시 분당구 서현동', '성남시 수정구 신흥동',
      '수원시 영통구 영통동', '수원시 팔달구 인계동', '용인시 수지구 풍덕천동',
      '고양시 일산동구 장항동', '고양시 덕양구 화정동', '부천시 원미구 중동', '안양시 동안구 평촌동',
      '의정부시 의정부동', '남양주시 다산동', '하남시 미사동', '광명시 철산동', '김포시 장기동', '시흥시 정왕동',
      // ── 인천 ──
      '인천광역시 부평구 부평동', '인천광역시 남동구 구월동', '인천광역시 연수구 송도동', '인천광역시 미추홀구 주안동'
    ];
    // 시/구 단위 상위 행정구역 추출 (예: "안산시", "안산시 상록구") → "안산" 입력 시도 노출
    var REGIONS = (function () {
      var set = {};
      DB.forEach(function (d) {
        var p = d.split(' ');
        // 시 단위 (예: 안산시 / 서울특별시 중구)
        if (p[0].match(/시$|광역시$|특별시$/)) {
          set[p[0]] = true;                                   // 안산시
          if (p[1]) set[p[0] + ' ' + p[1]] = true;            // 안산시 상록구 / 서울특별시 중구
        }
      });
    ...
    input.addEventListener('input', function () {
      ...
      var nq = v.replace(/\s/g, '');
      var norm = function (d) { return d.replace(/\s/g, ''); };
      // 동 단위 정확 매칭 + 시/구 단위 매칭을 합쳐서 노출 (시/구 우선)
      var regionM = REGIONS.filter(function (d) { return norm(d).indexOf(nq) > -1; });
      var dongM = DB.filter(function (d) { return norm(d).indexOf(nq) > -1; });
      var matches = regionM.concat(dongM).filter(function (m, i, arr) { return arr.indexOf(m) === i; }).slice(0, 10);
