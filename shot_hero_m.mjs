...
for (const sel of ['text=닫기', 'text=오늘 하루 보지 않기', '[aria-label="close"]']) {
  try { const el = await p.$(sel); if (el) { await el.click(); await p.waitForTimeout(300); break; } } catch(e){}
}
await p.waitForTimeout(600);
// scroll a bit so hero image (below headline on mobile) is visible
await p.evaluate(() => window.scrollTo(0, 350));
await p.waitForTimeout(500);
await p.screenshot({ path: '/tmp/hero_mobile2.png' });
await p.close();
await b.close();
console.log('done');
